...
def copyJsonFile2Excel(jsonFile,excelFile):
    ...
    worksheet.write(row+2,col,'Number of lines from json file:')
    ...
    worksheet.write(row,col+7,'Response',headersFormat)
    worksheet.write(row,col+8,'Response Time',headersFormat)
    ...
    while jf_line:
        ...
        for nsEntry in json_dict['queryResults']:
            for query in json_dict['queryResults'][nsEntry]:
                ...
                worksheet.write(row,col+6,query['query'])
                worksheet.write(row,col+7,','.join(query['response']))
                worksheet.write_number(row,col+8,float(query['responseTime']),numberFormat)
